...
| 모듈 | 시나리오 | 기대 | 검증 |
|------|----------|------|------|
| **two-pc** | 정상 `createMember` | 양쪽 DB 모두 커밋 | `MemberMigrationServiceTest > 정상 흐름` ✅ |
| **two-pc** | `createMemberThenFail` (저장 직전 예외) | 양쪽 모두 롤백(데이터 없음) | `> 롤백 흐름` ✅ |
| **saga** | `createMember` | Step1+2 완료 → COMPLETED | `MemberMigrationSagaTest > 정상 흐름` ✅ |
| **saga** | `createMemberThenFail` | Step2 실패 → Step1 보상(DELETE) → COMPENSATED, 양쪽 없음 | `> 보상 흐름` ✅ |

> two-pc 테스트는 **실제 MySQL XA**(Testcontainers 컨테이너 2개)로 Atomikos 가 동작한다 (H2 XA는 검증이 느슨함).
...
- `createMemberThenFail` → legacy 저장 직전 예외 → **양쪽 롤백** (테스트로 검증)
...
- `Step 1(신규 INSERT) → Step 2(기존 INSERT)`; Step 2 실패 시 **Step 1 보상(DELETE)** (테스트로 검증)
- 블로킹 없이 **보상 가능** → 일시적 중간 상태를 감수하는 대신 가용성·장애 격리 확보
